...
export default async (req: NextApiRequest, res: NextApiResponse) => {
	if (req.method === 'POST') {
		// -- on the server side you are not able to use the hook useSession, because hooks only works on the client string_decoder
		const session = await getSession({ req });

		// -- avoiding duplication of user on Stripe
